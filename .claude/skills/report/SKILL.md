...
2. For each file, read `status:` from the frontmatter. Count per project: `done`, `todo`, `dispatched`, `in-review`, `changes-requested`. Total = sum of all statuses. Report any other value as a legacy/invalid state instead of treating it as part of the active state machine.
