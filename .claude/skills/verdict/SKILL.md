...
2. Tìm task theo path/ID trong `$ARGUMENTS`.
3. Parse: verdict (`pass` hoặc `changes`), `--reviewer @id` (bắt buộc), `--commit <hash>` (bắt buộc nếu `pass`), `--notes "..."` (bắt buộc nếu `changes`).
4. Kiểm tra `status` hiện tại: chỉ chấp nhận verdict cho task đang `status: in-review`. Nếu khác → dừng lại, báo User (vd task chưa qua `/review-order`, hoặc đã `done` rồi).

### Bước 2 — Kiểm four-eyes (BẮT BUỘC, không được bỏ qua)

So `--reviewer` với `👷 executor` đã ghi trong task:
...
3. Đánh dấu task cha và toàn bộ sub-task liên quan thành `- [x]`.
4. Cập nhật metadata: `status: done`, `🔎 reviewer: <--reviewer>`.
5. Nếu task có khai báo `depends-on` (cú pháp chưa chính thức — xem `AGENTS.md` mục 2.2): nêu cho User những task nào có thể mở khóa tiếp theo, vì hiện chưa có cơ chế tự động parse/mở khóa — không tự suy diễn.
...
2. Thêm khối "Findings từ reviewer" vào task (dưới `▸ Plan:` hoặc cuối task), liệt kê từng ý trong `--notes` thành sub-task rework dạng `- [ ]`.
3. Cập nhật `status: changes-requested`. Giữ nguyên `👷 executor` (mặc định executor cũ sẽ sửa lại) trừ khi User nói đổi executor.
