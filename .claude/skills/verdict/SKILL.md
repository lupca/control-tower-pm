---
name: verdict
description: Record a reviewer's verdict — pass closes task (needs human confirmation), changes reopens with findings. Checks four-eyes. Only Markdown, never code/tests. Activate on /verdict.
...
Compare `--reviewer` vs `executor:` in frontmatter. If same → **REFUSE**. If different → continue.
...
3. **Human confirmation required** (`AGENTS.md` §3, §4). User typing `/verdict pass` = confirmation. Agent proposing it = STOP and ask.
...
8. Write 1 entry to `log.md` (format: `AGENTS-REFERENCE.md` §7).
