...
2. Đọc toàn bộ `inbox.md`. Với mỗi ghi chú thô:
   ...
   d. Nếu chưa có task tương tự, áp dụng đúng quy trình graph như `.claude/skills/pm/references/task-creation.md`: `get_minimal_context_tool` → `semantic_search_nodes_tool`/`get_impact_radius_tool` → `query_graph_tool(pattern="tests_for", target=...)`, luôn kèm `repo_root` và `detail_level="minimal"`, để xác nhận path thật thay cho path đoán mò ghi trong ghi chú thô. Đọc `_project.md` lấy `task_prefix`/`next_task_id`, tạo file `projects/<tên>/tasks/<ID>-<slug>.md` đúng cú pháp `AGENTS.md` mục 2.1 (có `files:`/AC/`tests:`, `status: todo`), tăng `next_task_id`.
