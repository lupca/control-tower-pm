...
7. `get_knowledge_gaps_tool(repo_root=...)` — if the impacted area touches an uncovered hotspot, add a sub-task:
   `- [ ] Write a test for <symbol/file> (currently no coverage — knowledge gap) — suggested test file: <suggested test file>`
8. `get_hub_nodes_tool(top_n=50, repo_root=...)` and `get_bridge_nodes_tool(top_n=50, repo_root=...)` — if any node in `files:` matches the returned list → flag `risk: high` in the frontmatter.
9. `get_affected_flows_tool(changed_files=[...], repo_root=...)` → fills in `flows:`.
10. **Compute Pre-Execution Prediction Score (`predicted_success`)**:
