...
11. **LLM-Modulo verifier**: evaluate every rule in `.agents/verifier-rules.yaml` against the draft task, print the results block. Any `❌` needs a mechanical auto-fix or an explicit User override (recorded in `## Verifier Overrides`) before the Spec Gate can close below.
