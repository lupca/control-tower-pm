...
- Mọi thay đổi liên quan đến cấu trúc DB hoặc luồng thanh toán/hóa đơn bắt buộc phải có sự xác nhận của User trước khi Subagent chạy migrate hoặc đụng tới logic tiền.
- Các task hoàn thành phải pass qua 100% test case trong file test tương ứng mới được đánh dấu `- [x]`.
- Test chạy trong Docker: `docker compose -f OMS/docker-compose.yml exec api pytest ...`.
